...
/* =========================================================
 * 초음파 거리 로그 (UART6, non-blocking DMA)
 * ---------------------------------------------------------
 * 테스트용 함수. 실제 호출은 주석 처리되어 있음.
 * 사용 시 호출부 주석 해제할 것.
 *
 * 출력 형식:
 *   [US] L: 45cm  C: 80cm  R: 32cm
 * ========================================================= */
void SHOW_UART6_ULTRASONIC(void)
{
    static uint32_t prev_time_us = 0U;
    uint32_t now;
    int len;
    uint16_t tx_len;
    ...
    len = snprintf(uart6_msg, BMS_UART6_MSG_SIZE,
                   "[US] L:%3ucm  C:%3ucm  R:%3ucm\r\n",
                   (unsigned)Ultrasonic_GetDistanceCm(US_LEFT),
                   (unsigned)Ultrasonic_GetDistanceCm(US_CENTER),
                   (unsigned)Ultrasonic_GetDistanceCm(US_RIGHT));

    tx_len = BMS_MESSAGE_GetSafeLen(len, BMS_UART6_MSG_SIZE);
    if (tx_len == 0U)
    {
        return;
    }
    ...
    if (HAL_UART_Transmit_DMA(&huart6, (uint8_t *)uart6_msg, tx_len) != HAL_OK)
    {
        uart6_tx_busy = 0U;
    }
}
